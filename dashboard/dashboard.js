...
(function () {
  ...
  function getCutoffDate(filter) {
    ...
    var d = new Date(now);
    switch (filter) {
      case 'thisWeek':     d.setDate(now.getDate() - 7);    break;
      case 'last3months':  d.setMonth(now.getMonth() - 3);   break;
  ...
  function buildOptions(override) {
    ...
    var opts = {};
    var key;
    for (key in D) { if (D.hasOwnProperty(key)) opts[key] = D[key]; }
    ...
    opts.scales.x = Object.assign(
      {}, D.scales.x, theme.x, srcScales.x || {}
    );
    opts.scales.y = Object.assign(
      {}, D.scales.y, theme.y, srcScales.y || {}
    );
    ...
    } else if (opts.plugins.legend && opts.plugins.legend.labels && !opts.plugins.legend.labels.color) {
      opts.plugins.legend.labels.color = text;
    }
  ...
  function showElem(id)  { var e = document.getElementById(id); if (e) e.classList.remove('hidden'); }
  function hideElem(id)  { var e = document.getElementById(id); if (e) e.classList.add('hidden'); }

  function showLoading(tab)  { showElem(tab + '-loading'); }
  function hideLoading(tab)  { hideElem(tab + '-loading'); }
  function showError(tab, msg) {
    var el = document.getElementById(tab + '-error');
    if (el) { el.textContent = msg; showElem(tab + '-error'); }
  }
  function showEmpty(tab) { showElem(tab + '-empty'); }
  ...
  function loadData() {
    var fetches = [
      ...
      fetch('/data/contributions.json').then(function (r) { return r.json(); }),
      fetch('/data/contributors.json').then(function (r) { return r.json(); }),
      fetch('/data/frequency.json').then(function (r) { return r.json(); }),
      fetch('/data/activity.json').then(function (r) { return r.json(); }),
    ];
    ...
    return Promise.all(fetches).then(function (results) {
      ...
      TABS.forEach(function (t) {
        showError(t, 'Failed to load dashboard data. Ensure the server is running and the repository has been analyzed.');
      });
      throw err;
    });
  }
  ...
  function renderCurrentTab() {
    ...
    switch (state.activeTab) {
      ...
      case 'contributors':  renderContributors(d);  break;
      case 'codefrequency': renderCodeFrequency(d); break;
      case 'activity':      renderActivity(d);      break;
    }
  }
  ...
  function renderContributionChart(contributions) {
    ...
    contributions.forEach(function (day) {
      (day.authorDetails || []).forEach(function (a) {
        totals[a.author] = (totals[a.author] || 0) + a.count;
      });
    });
    ...
    var datasets = [];
    topAuthors.forEach(function (author, i) {
      var lookup = authorDayIndex[author];
      datasets.push({
        label: author,
  ...
  function renderFrequencyMiniChart(frequency) {
    ...
    createChart('chart-frequency-mini', 'line', {
  ...
  function renderContributors(d) {
    ...
    d.contributors.forEach(function (c) {
      ...
      tr.innerHTML =
        ...
        '<td>' + formatDate(c.firstCommit) + '</td>' +
        '<td>' + formatDate(c.lastCommit) + '</td>';
  ...
  function renderCodeFrequency(d) {
    ...
    createChart('chart-frequency', 'line', {
      labels: freq.map(function (x) { return x.date; }),
      datasets: [
        {
          label: 'Additions',
          data: freq.map(function (x) { return x.additions; }),
          borderColor:     window.COLORS.green,
          backgroundColor: window.COLORS.green + '30',
          fill: true, tension: 0.3,
          pointRadius: freq.length < 60 ? 2 : 0, pointHoverRadius: 4, borderWidth: 2,
        },
        {
          label: 'Deletions',
          data: freq.map(function (x) { return x.deletions; }),
          borderColor:     window.COLORS.red,
  ...
  function renderActivity(d) {
    ...
    var a = d.activity;
    if (!a) { showEmpty('activity'); return; }

    var hasWeek  = a.byDayOfWeek && a.byDayOfWeek.some(function (x) { return x.count > 0; });
    var hasHour  = a.byHour && a.byHour.some(function (x) { return x.count > 0; });
    ...
    if (a.byDayOfWeek && a.byDayOfWeek.length) {
      createChart('chart-dayofweek', 'bar', {
        labels: a.byDayOfWeek.map(function (x) { return x.day; }),
        datasets: [{
          label: 'Commits',
          data: a.byDayOfWeek.map(function (x) { return x.count; }),
    ...
    if (a.byHour && a.byHour.length) {
      createChart('chart-hour', 'bar', {
        labels: a.byHour.map(function (x) {
          return String(x.hour).padStart(2, '0') + ':00';
        }),
        datasets: [{
          label: 'Commits',
          data: a.byHour.map(function (x) { return x.count; }),
    ...
    if (a.topFiles && a.topFiles.length) {
      ...
      a.topFiles.forEach(function (f) {
        ...
        tr.innerHTML =
          ...
          '<td class="num-col">' + formatNumber(f.changes) + '</td>';
        tbody.appendChild(tr);
      });
    }
  }
  ...
  function init() {
    ...
    TABS.forEach(function (t) { showLoading(t); });

    loadData().then(function () {
      TABS.forEach(function (t) { clearStates(t); });
      renderCurrentTab();
    }).catch(function () {
